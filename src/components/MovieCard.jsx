...
export const MovieCard = ({ movie}) => {
  ...
  return (
    ...
    <Link to={`/movie/${id}`} className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600 m-3">
      ...
      <div className="flex flex-col justify-between p-2 leading-normal lg:min-w-[366px]">
          <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{original_title}</h5>
          <p className="overview  mb-1 font-normal text-gray-700 dark:text-gray-400 md:max-h-[191px] md:max-w-[356px] overflow-hidden text-ellipsis">{overview}</p>
      </div>
    </Link>
  )
}
